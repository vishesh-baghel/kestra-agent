...
async function initializePluginCache(): Promise<PluginCache> {
  if (pluginCache && (new Date().getTime() - pluginCache.lastUpdated.getTime()) < 3600000) {
    // Cache is less than 1 hour old, return it
    return pluginCache;
  }

  try {
    console.log("Initializing Kestra plugin cache...");
    ...
    console.log(`Plugin cache initialized with ${plugins.length} plugins and ${taskIndex.size} task name mappings`);
    return pluginCache;
  } catch (error) {
    console.error("Failed to initialize plugin cache:", error);
...
function findTaskByName(query: string, cache: PluginCache): string | null {
  ...
  cache.taskIndex.forEach((fullClassName, indexName) => {
    ...
    if (indexName === query) score += 100;  // Exact match
    else if (indexName.startsWith(query)) score += 50;  // Prefix match
 ...
 */
async function fetchTaskDocumentation(fullClassName: string): Promise<PluginDocumentation | null> {
  ...
  try {
    console.log(`Fetching documentation for ${fullClassName}`);
    const response = await axios.get(`${KESTRA_API_BASE_URL}/plugins/${fullClassName}`);
    ...
    console.error(`Failed to fetch documentation for ${fullClassName}:`, error);
    return null;
  }
}
 ...
 */
function processTaskDocumentation(fullClassName: string, doc: PluginDocumentation): TaskDocInfo {
  // Extract description from schema
  const description = doc.schema.properties?.description || "No description available";
  ...
  const packagePrefix = fullClassName.substring(0, fullClassName.lastIndexOf('.'));
...
export const kestraDocsTool = createTool({
  ...
  outputSchema: z.object({
    ...
    taskOutputs: z.record(z.string()).optional().describe("Task output properties"),
...
initializePluginCache().catch(err => {
  console.error("Failed to initialize Kestra plugin cache:", err);
});
